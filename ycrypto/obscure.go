...
func Reveal(x string) (string, error) {
	...
	if err != nil {
		return "", fmt.Errorf("base64 decode failed when revealing password: %w", err)
	}
	if len(ciphertext) < aes.BlockSize {
		return "", fmt.Errorf("input too short when revealing password")
	}
	buf := ciphertext[aes.BlockSize:]
	iv := ciphertext[:aes.BlockSize]
	if err := crypt(buf, buf, iv); err != nil {
		return "", fmt.Errorf("decrypt failed when revealing password: %w", err)
	}
